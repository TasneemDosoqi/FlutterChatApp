...
class Auth {
  ...
  Future SignInEandP(String email, String passw) async{
    try{
      AuthResult result = await _auth.signInWithEmailAndPassword(email: email, password: passw);
  ...
  Future SignUpEandP (String email , String paasw)async{
    try{
      AuthResult result = await _auth.createUserWithEmailAndPassword(email: email, password: paasw);
